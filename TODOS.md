...
**Context:** Google shipped Chrome DevTools MCP in Chrome 146+ (June 2025). It provides screenshots, console messages, performance traces, Lighthouse audits, and full page interaction through the user's real browser. opengstack should use it for real-session access while keeping Playwright for headless CI/testing workflows.
...
`$B state save/load` ships in v0.12.1.0. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Files at `.OpenGStack/browse-states/{name}.json` with 0o600 permissions. Load replaces session (closes all pages first). Name sanitized to `[a-zA-Z0-9_-]`.
...
`$B connect` launches Playwright's bundled Chromium in headed mode with the opengstack Chrome extension auto-loaded. `$B handoff` now produces the same result (extension + side panel). Sidebar chat gated behind `--chat` flag.
...
**What:** Publish the opengstack browse Chrome extension to Chrome Web Store for easier install.
...
**Why:** The bug fixed in v0.11.22 ( showed that /ship's CHANGELOG generation biased toward recent commits on long branches. The prompt fix adds a cross-check, but no test exercises the multi-commit failure mode. The existing `ship-local-workflow` E2E only uses a single-commit branch.
...
**What:** Append structured JSON entry to `.OpenGStack/ship-log.json` at end of every /ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).
...
**Effort:** M
**Priority:** P2
**Depends on:** /setup-opengstack-upload
...
### /setup-opengstack-upload skill (S3 bucket)
...
### opengstack-upload helper

**What:** `browse/bin/opengstack-upload` — upload file to S3, return public URL.
...
**Context:** Reads `~/.opengstack-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**What:** `opengstack-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If opengstack ever supports Linux, this silently fails.
...
**Context:** /office-hours writes design docs to `~/.opengstack/projects/$SLUG/`. The team store already syncs test plans, retro snapshots, QA reports. Design docs follow the same pattern — just add a sync adapter.
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the opengstack voice guide (friendly, user-forward, lead with benefits). Inject into /ship Step 5, /document-release Step 5, and reference from CLAUDE.md.
...
`bin/opengstack-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** opengstack-diff-scope (shipped)

## Codex

### Codex→Claude reverse buddy check skill

**What:** A Codex-native skill (`.agents/skills/opengstack-claude/SKILL.md`) that runs `claude -p` to get an independent second opinion from Claude — the reverse of what `/codex` does today from Claude Code.
...
**Context:** The `/codex` skill template (`codex/SKILL.md.tmpl`) shows the pattern — it wraps `codex exec` with JSONL parsing, timeout handling, and structured output. The reverse skill would wrap `claude -p` with similar infrastructure. Would be generated into `.agents/skills/opengstack-claude/` by `gen-skill-docs --host codex`.
...
**What:** Track how often Claude chooses the complete option vs shortcut across opengstack sessions. Aggregate into a dashboard showing completeness trend over time.
...
5. Debug session persistence (~/.opengstack/investigate-sessions/ — save investigation for reuse)
...
**What:** Expose OpenGStack's browse binary and key workflows as an MCP server that Factory Droid connects to natively. Factory users would run /mcp, add the opengstack server, and get browse, QA, and review capabilities as Factory tools.

**Why:** Factory already supports 40+ MCP servers in its registry. Getting OpenGStack's browse binary listed there is a distribution play. Nobody else has a real compiled browser binary as an MCP tool. This is the thing that makes opengstack uniquely valuable on Factory Droid.
...
**What:** Factory has "custom droids" (subagents with tool restrictions, model selection, autonomy levels). Could ship `opengstack-qa.md` droid configs alongside skills that restrict tools to read-only + execute for safety.

**Why:** Deeper Factory integration. Droid configs give Factory users tighter control over what opengstack skills can do.
...
- Config CLI (`bin/opengstack-config`), auto-upgrade via `~/.opengstack/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
**Completed:** v0.3.8
