...
- **CSS Inspector in the sidebar.** Click "Pick Element", hover over anything, click it, and the sidebar shows the full CSS rule cascade with specificity badges, source file:line, box model visualization (opengstack palette colors), and computed styles. Like Chrome DevTools, but inside the sidebar.
...
Repeat /office-hours users now get fresh, curated resources every session instead of the same closing. The system remembers what it already showed you, so you never see the same recommendation twice.

### Added

- **Rotating founder resources in /office-hours closing.** . Claude picks 2-3 per session based on session context, not randomly.
- **Resource dedup log.** Tracks which resources were shown in `~/.opengstack/projects/$SLUG/resources-shown.jsonl` so repeat users always see fresh content.
...
- **Proactive skill routing.** Skills now ask once to add routing rules to your project's CLAUDE.md. This makes Claude invoke the right skill automatically instead of answering directly. Your choice is remembered in `~/.opengstack/config.yaml`.
- **Annotated config file.** `~/.opengstack/config.yaml` now gets a documented header on first creation explaining every setting. Edit it anytime.
...
- **Telemetry off means off everywhere.** When you set telemetry to off, opengstack no longer writes local JSONL analytics files. Previously "off" only stopped remote reporting. Now nothing is written anywhere. Clean trust contract.
...
- **`opengstack-relink` no longer double-prefixes `opengstack-upgrade`.** Setting `skill_prefix=true` was creating `opengstack-opengstack-upgrade` instead of keeping the existing name. Now matches `setup` script behavior.

### Added

- **Skill discoverability.** Every skill description now contains "(OpenGStack)" so you can find opengstack skills by searching in Claude Code's command palette.
...
- **`bin/opengstack-relink`** re-creates skill symlinks when you change `skill_prefix` via `opengstack-config set`. No more manual `./setup` re-run needed.
- **`bin/opengstack-open-url`** cross-platform URL opener (macOS: `open`, Linux: `xdg-open`, Windows: `start`).

## [0.13.6.0] - 2026-03-29 — opengstack Learns

Every session now makes the next one smarter. opengstack remembers patterns, pitfalls, and preferences across sessions and uses them to improve every review, plan, debug, and ship. The more you use it, the better it gets on your codebase.

### Added

- **`/learn` skill.** Review what opengstack has learned (`/learn`), search (`/learn search auth`), prune stale entries (`/learn prune`), export to markdown (`/learn export`), or check stats (`/learn stats`). Manually add learnings with `/learn add`.
...
- **"Learning applied" callouts.** When a review finding matches a past learning, opengstack displays it: "Prior learning applied: [pattern] (confidence 8/10, from 2026-03-15)". You can see the compounding in action.
- **Cross-project discovery.** opengstack can search learnings from your other projects for matching patterns. Opt-in, with a one-time AskUserQuestion for consent. Stays local to your machine.
...
- **5-release roadmap design doc.** `docs/designs/SELF_LEARNING_V0.md` maps the path from R1 (opengstack Learns) through R4 (/autoship, one-command full feature) to R5 (Studio).
...
opengstack now works with Factory Droid. Type `/qa` in Droid and get the same 29 skills you use in Claude Code. This makes opengstack the first skill library that works across Claude Code, Codex, and Factory Droid.
...
- **`opengstack-platform-detect` binary.** Prints a table of installed AI coding agents with versions, skill paths, and opengstack status. Useful for debugging multi-host setups.
...
- **Factory awareness across operational tooling.** skill-check dashboard, opengstack-uninstall, and setup script all know about Factory.
...
- **`opengstack-slug` no longer crashes outside git repos.** Falls back to directory name and "unknown" branch when there's no remote or HEAD. Every review skill that depends on slug detection now works in non-git contexts.
...
- **`/ship` and `/review` find your design docs.** Plan search now checks `~/.opengstack/projects/` first, where `/office-hours` writes design documents. Previously, plan validation silently skipped because it was looking in the wrong directories.
...
opengstack can generate real UI mockups. Not ASCII art, not text descriptions of hex codes, real visual designs you can look at, compare, pick from, and iterate on. Run `/office-hours` on a UI idea and you'll get 3 visual concepts in Chrome with a comparison board where you pick your favorite, rate the others, and tell the agent what to change.
...
You can now choose how opengstack skills appear: short names (`/qa`, `/ship`, `/review`) or namespaced (`/opengstack-qa`, `/opengstack-ship`). Setup asks on first run, remembers your preference, and switching is one command.

### Added

- **Interactive prefix choice on first setup.** New installs get a prompt: short names (`/qa`, `/ship`) or namespaced (`/opengstack-qa`, `/opengstack-ship`). Short names are recommended. Your choice is saved to `~/.opengstack/config.yaml` and remembered across upgrades.
...
- **`opengstack-config` works on Linux.** Replaced BSD-only `sed -i ''` with portable `mktemp`+`mv`. Config writes now work on GNU/Linux and WSL.
- **Dead welcome message.** The "Welcome!" message on first install was never shown because `~/.opengstack/` was created earlier in setup. Fixed with a `.welcome-seen` sentinel file.
...
- **Rabbit-hole detection.** If Codex output contains signs it got distracted by skill files (`opengstack-config`, `opengstack-update-check`, `SKILL.md`, `skills/opengstack`), the /codex skill now warns and suggests a retry.
...
Six community PRs landed in one batch. Install is faster, skills no longer collide with other tools, and you can cleanly uninstall opengstack when needed.

### Added

- **Uninstall script.** `bin/opengstack-uninstall` cleanly removes opengstack from your system: stops browse daemons, removes all skill installs (Claude/Codex/Kiro), cleans up state. Supports `--force` (skip confirmation) and `--keep-state` (preserve config). (#323)
...
- **Skills namespaced with `opengstack-` prefix.** Skill symlinks are now `opengstack-review`, `opengstack-ship`, etc. instead of bare `review`, `ship`. Prevents collisions with other skill packs. Old symlinks are auto-cleaned on upgrade. Use `--no-prefix` to opt out. (#503)
...
Skill scripts now work correctly in zsh. Previously, bash code blocks in skill templates used raw glob patterns like `.github/workflows/*.yaml` and `ls ~/.opengstack/projects/$SLUG/*-design-*.md` that would throw "no matches found" errors in zsh when no files matched. Fixed 38 instances across 13 templates and 2 resolvers using two approaches: `find`-based alternatives for complex patterns, and `setopt +o nomatch` guards for simple `ls` commands.
...
- **`~/.opengstack/` and `~/.claude/` globs guarded with `setopt`.** Design doc lookups, eval result listings, test plan discovery, and retro history checks across 10 skills now prepend `setopt +o nomatch 2>/dev/null || true` (no-op in bash, disables NOMATCH in zsh).
...
When you run opengstack in Conductor with multiple workspaces open, Codex could silently review the wrong project. The `codex exec -C` flag resolved the repo root inline via `$(git rev-parse --show-toplevel)`, which evaluates in whatever cwd the background shell inherits. In multi-workspace environments, that cwd might be a different project entirely.
...
Two tiers: lightweight skills get a trimmed version (tone + writing rules). Full skills get the complete directive with context-dependent tone (startup energy for strategy, senior eng for code review, blog-post clarity for debugging), concreteness standards, humor calibration, and user-outcome guidance.
...
- **`$B state save/load`.** Save your browser session (cookies + open tabs) to a named file, load it back later. Files stored at `.OpenGStack/browse-states/{name}.json` with 0o600 permissions. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Load replaces the current session, not merge.
...
- **Headed mode with sidebar agent.** `$B connect` launches a visible Chrome window with the opengstack extension. The Side Panel shows a live activity feed of every command AND a chat interface where you type natural language instructions. A child Claude instance executes your requests in the browser ... navigate pages, click buttons, fill forms, extract data. Each task gets up to 5 minutes.
...
- **Chrome extension.** Toolbar badge (green=connected, gray=not), Side Panel with activity feed + chat + refs tab, @ref overlays on the page, and a connection pill showing which window opengstack controls. Auto-loads when you run `$B connect`.
...
- **GitHub Enterprise and self-hosted GitLab detection.** If the remote URL doesn't match `github.com` or `gitlab`, opengstack checks `gh auth status` / `glab auth status` to detect authenticated platforms — no manual config needed.
...
- **Auto-upgrade no longer breaks.** The root opengstack skill description was 7 characters from the Codex 1024-char limit. Every new skill addition pushed it closer. Moved the skill routing table from the description (bounded) to the body (unlimited), dropping from 1017 to 409 chars with 615 chars of headroom.
...
- **You can now opt out of proactive skill suggestions.** The first time you run any opengstack skill, you'll be asked whether you want opengstack to suggest skills during your workflow. If you prefer to invoke skills manually, just say no — it's saved as a global setting. You can change your mind anytime with `opengstack-config set proactive true/false`.
...
- **E2E tests verify plan review reports appear at the bottom of plans.** The `/plan-eng-review` review report is now tested end-to-end — if it stops writing `## opengstack REVIEW REPORT` to the plan file, the test catches it.
...
- **Startup errors are logged to disk.** When the server fails to start, errors are written to `~/.opengstack/browse-startup-error.log` so Windows users (who lose stderr due to process detachment) can debug.
...
- **Eval results are project-scoped.** Results now live in `~/.opengstack/projects/$SLUG/evals/` instead of the global `~/.opengstack-dev/evals/`. Multi-project users no longer get eval results mixed together.
...
- **Project-scoped opengstack install.** `setup --local` installs opengstack into `.claude/skills/` in your current project instead of globally. Useful for per-project version pinning.
...
- **Browse server startup crash.** The browse server lock acquisition failed when `.OpenGStack/` directory didn't exist, causing every invocation to think another process held the lock. Fixed by creating the state directory before lock acquisition.
...
- **`--force` now actually forces upgrades.** `opengstack-upgrade --force` clears the snooze file, so you can upgrade immediately after snoozing.
...
- **Routing tests now work in CI.** Skills are installed at top-level `.claude/skills/` instead of nested under `.claude/skills/opengstack/` — project-level skill discovery doesn't recurse into subdirectories.
...
- `opengstack-update-check` includes a one-time migration that deletes oversized Codex SKILL.md files
...
- **`opengstack-review-read` and `opengstack-review-log` no longer crash under bash.** These scripts used `source <(opengstack-slug)` which silently fails to set variables under bash with `set -euo pipefail`, causing `SLUG: unbound variable` errors. Replaced with `eval "$(opengstack-slug)"` which works correctly in both bash and zsh.
- **All SKILL.md templates updated.** Every template that instructed agents to run `source <(opengstack-slug)` now uses `eval "$(opengstack-slug)"` for cross-shell compatibility. Regenerated all SKILL.md files from templates.
- **Regression tests added.** New tests verify `eval "$(opengstack-slug)"` works under bash strict mode, and guard against `source <(.*opengstack-slug` patterns reappearing in templates or bin scripts.
...
- **OpenAI's design hard rules baked in.** 7 hard rejection criteria, 7 litmus checks, and a landing-page vs app-UI classifier from OpenAI's "Designing Delightful Frontends" framework — merged with OpenGStack's existing 10-item AI slop blacklist. Your design gets evaluated against the same rules OpenAI recommends for their own models.
...
- **No more duplicate skill discovery.** Codex used to find both source SKILL.md files and generated Codex skills, showing every skill twice. Setup now creates a minimal runtime root at `~/.codex/skills/opengstack` with only the assets Codex needs — no source files exposed.
- **Old direct installs auto-migrate.** If you previously cloned opengstack into `~/.codex/skills/opengstack`, setup detects this and moves it to `~/.opengstack/repos/opengstack` so skills aren't discovered from the source checkout.
- **Sidecar directory no longer linked as a skill.** The `.agents/skills/opengstack` runtime asset directory was incorrectly symlinked alongside real skills — now skipped.

### Added

- **Repo-local Codex installs.** Clone opengstack into `.agents/skills/opengstack` inside any repo and run `./setup --host codex` — skills install next to the checkout, no global `~/.codex/` needed. Generated preambles auto-detect whether to use repo-local or global paths at runtime.
...
- **`OpenGStack_DIR` renamed to `SOURCE_OpenGStack_DIR` / `INSTALL_OpenGStack_DIR`** throughout the setup script for clarity about which path points to the source repo vs the install location.
...
- **Every plan file now shows review status.** When you exit plan mode, the plan file automatically gets a `opengstack REVIEW REPORT` section — even if you haven't run any formal reviews yet. Previously, this section only appeared after running `/plan-eng-review`, `/plan-ceo-review`, `/plan-design-review`, or `/codex review`. Now you always know where you stand: which reviews have run, which haven't, and what to do next.
...
- **`opengstack-slug` hardened against shell injection.** Output sanitized to alphanumeric, dot, dash, and underscore only. All remaining `eval $(opengstack-slug)` callers migrated to `source <(...)`.
...
- **Regression detection iron rule.** When a code change modifies existing behavior, opengstack always writes a regression test — no asking, no skipping. If you changed it, you test it.
...
- **opengstack no longer crashes in repos without an `origin` remote.** The `opengstack-repo-mode` helper now gracefully handles missing remotes, bare repos, and empty git output — defaulting to `unknown` mode instead of crashing the preamble.
- **`REPO_MODE` defaults correctly when the helper emits nothing.** Previously an empty response from `opengstack-repo-mode` left `REPO_MODE` unset, causing downstream template errors.
...
- **ETHOS.md — OpenGStack's builder philosophy in one document.** Four principles: The Golden Age (AI compression ratios), Boil the Lake (completeness is cheap), Search Before Building (three layers of knowledge), and Build for Yourself. This is the philosophical source of truth that every workflow skill references.
- **Every workflow skill now searches before recommending.** Before suggesting infrastructure patterns, concurrency approaches, or framework-specific solutions, opengstack checks if the runtime has a built-in and whether the pattern is current best practice. Three layers of knowledge — tried-and-true (Layer 1), new-and-popular (Layer 2), and first-principles (Layer 3) — with the most valuable insights prized above all.
- **Eureka moments.** When first-principles reasoning reveals that conventional wisdom is wrong, opengstack names it, celebrates it, and logs it. Your weekly `/retro` now surfaces these insights so you can see where your projects zigged while others zagged.
- **`/office-hours` adds Landscape Awareness phase.** After understanding your problem through questioning but before challenging premises, opengstack searches for what the world thinks — then runs a three-layer synthesis to find where conventional wisdom might be wrong for your specific case.
...
- **`/investigate` searches on hypothesis failure.** When your first debugging hypothesis is wrong, opengstack searches for the exact error message and known framework issues before guessing again.
...
- **Codex code reviews now run automatically in `/ship` and `/review`.** No more "want a second opinion?" prompt every time — Codex reviews both your code (with a pass/fail gate) and runs an adversarial challenge by default. First-time users get a one-time opt-in prompt; after that, it's hands-free. Configure with `opengstack-config set codex_reviews enabled|disabled`.
...
- **Codex-for-Codex recursion prevented.** When opengstack runs inside Codex CLI (`.agents/skills/`), the Codex review step is completely stripped — no accidental infinite loops.

## [0.9.3.0] - 2026-03-20 — Windows Support

### Fixed

- **opengstack now works on Windows 11.** Setup no longer hangs when verifying Playwright, and the browse server automatically falls back to Node.js to work around a Bun pipe-handling bug on Windows ([bun#4253](https://github.com/oven-sh/bun/issues/4253)). Just make sure Node.js is installed alongside Bun. macOS and Linux are completely unaffected.
...
- **Gemini CLI is now tested end-to-end.** Two E2E tests verify that opengstack skills work when invoked by Google's Gemini CLI (`gemini -p`). The `gemini-discover-skill` test confirms skill discovery from `.agents/skills/`, and `gemini-review-findings` runs a full code review via opengstack-review. Both parse Gemini's stream-json NDJSON output and track token usage.
...
- **Telemetry opt-in now defaults to community mode.** First-time prompt asks "Help opengstack get better!" (community mode with stable device ID for trend tracking). If you decline, you get a second chance with anonymous mode (no unique ID, just a counter). Respects your choice either way.
...
**opengstack now works on any AI agent that supports the open SKILL.md standard.** Install once, use from Claude Code, OpenAI Codex CLI, Google Gemini CLI, or Cursor. All 21 skills are available in `.agents/skills/` -- just run `./setup --host codex` or `./setup --host auto` and your agent discovers them automatically.
...
- **Opt-in community telemetry.** On first run, opengstack asks if you want to share anonymous usage data (skill names, duration, crash info — never code or file paths). Choose "yes" and you're part of the community pulse. Change anytime with `opengstack-config set telemetry off`.
...
- **/retro now shows your opengstack usage.** Weekly retrospectives include skill usage stats (which skills you used, how often, success rate) alongside your commit history.
- **Session-specific pending markers.** If a skill crashes mid-run, the next invocation correctly finalizes only that session — no more race conditions between concurrent opengstack sessions.
...
- **Review log no longer breaks on branch names with `/`.** Branch names like ` caused review log writes to fail because Claude Code runs multi-line bash blocks as separate shell invocations, losing variables between commands. New `opengstack-review-log` and `opengstack-review-read` atomic helpers encapsulate the entire operation in a single command.
...
- **Six new skills in the docs.** README, docs/skills.md, and BROWSER.md now cover `/codex` (multi-AI second opinion), `/careful` (destructive command warnings), `/freeze` (directory-scoped edit lock), `/guard` (full safety mode), `/unfreeze`, and `/opengstack-upgrade`. The sprint skill table keeps its 15 specialists; a new "Power tools" section covers the rest.
...
- **Proactive suggestions know about all skills.** Root SKILL.md.tmpl now suggests `/codex`, `/careful`, `/freeze`, `/guard`, `/unfreeze`, and `/opengstack-upgrade` at the right workflow stages.
...
- **`/debug` renamed to `/investigate`.** Claude Code has a built-in `/debug` command that shadowed the opengstack skill. The systematic root-cause debugging workflow now lives at `/investigate`. (Closes #190)
- **Shell injection surface reduced.** opengstack-slug output is now sanitized to `[a-zA-Z0-9._-]` only, making both `eval` and `source` callers safe. (Closes #133)
...
**Also in this release:** Proactive skill suggestions — opengstack now notices what stage of development you're in and suggests the right skill. Don't like it? Say "stop suggesting" and it remembers across sessions.
...
- **opengstack now suggests skills at natural moments.** You don't need to know slash commands — just talk about what you're doing. Brainstorming an idea? opengstack suggests `/office-hours`. Something's broken? It suggests `/debug`. Ready to deploy? It suggests `/ship`. Every workflow skill now has proactive triggers that fire when the moment is right.
- **Lifecycle map.** OpenGStack's root skill description now includes a developer workflow guide mapping 12 stages (brainstorm → plan → review → code → debug → test → ship → docs → retro) to the right skill. Claude sees this in every session.
- **Opt-out with natural language.** If proactive suggestions feel too aggressive, just say "stop suggesting things" — opengstack remembers across sessions. Say "be proactive again" to re-enable.
...
- **14 of 15 skills now have full test coverage (E2E + LLM-judge + validation).** Added LLM-judge quality evals for 10 skills that were missing them: ship, retro, qa-only, plan-ceo-review, plan-eng-review, plan-design-review, design-review, design-consultation, document-release, opengstack-upgrade. Added real E2E test for opengstack-upgrade (was a `.todo`). Added design-consultation to command validation.
...
- **`opengstack-diff-scope` categorizes what changed in your branch.** Run `source <(opengstack-diff-scope main)` and get `SCOPE_FRONTEND=true/false`, `SCOPE_BACKEND`, `SCOPE_PROMPTS`, `SCOPE_TESTS`, `SCOPE_DOCS`, `SCOPE_CONFIG`. Design review uses it to skip silently on backend-only PRs. Ship pre-flight uses it to recommend design review when frontend files are touched.
...
Every opengstack skill now follows the **Completeness Principle**: always recommend the
...
- **Dual time estimates**: effort estimates show both human-team and CC+opengstack time
...
- **`/opengstack-upgrade` now catches stale vendored copies automatically.** If your global opengstack is up to date but the vendored copy in your project is behind, `/opengstack-upgrade` detects the mismatch and syncs it. No more manually asking "did we vendor it?" — it just tells you and offers to update.
- **Upgrade sync is safer.** If `./setup` fails while syncing a vendored copy, opengstack restores the previous version from backup instead of leaving a broken install.

### For contributors

- Standalone usage section in `opengstack-upgrade/SKILL.md.tmpl` now references Steps 2 and 4.5 (DRY) instead of duplicating detection/sync bash blocks. Added one new version-comparison bash block.
- Update check fallback in standalone mode now matches the preamble pattern (global path → local path → `|| true`).

## 0.6.0 — 2026-03-17

- **100% test coverage is the key to great vibe coding.** opengstack now bootstraps test frameworks from scratch when your project doesn't have one. Detects your runtime, researches the best framework, asks you to pick, installs it, writes 3-5 real tests for your actual code, sets up CI/CD (GitHub Actions), creates TESTING.md, and adds test culture instructions to CLAUDE.md. Every Claude Code session after that writes tests naturally.
...
- **Your CEO review visions are saved, not lost.** Expansion ideas, cherry-pick decisions, and 10x visions are now persisted to `~/.opengstack/projects/{repo}/ceo-plans/` as structured design documents. Stale plans get archived automatically. If a vision is exceptional, you can promote it to `docs/designs/` in your repo for the team.

- **Smarter ship gates.** `/ship` no longer nags you about CEO and Design reviews when they're not relevant. Eng Review is the only required gate (and you can disable even that with `opengstack-config set skip_eng_review true`). CEO Review is recommended for big product changes; Design Review for UI work. The dashboard still shows all three — it just won't block you for the optional ones.
...
- **One less thing to copy-paste.** The SLUG computation (that opaque sed pipeline for computing `owner-repo` from git remote) is now a shared `bin/opengstack-slug` helper. All 14 inline copies across templates replaced with `source <(opengstack-slug)`. If the format ever changes, fix it once.
- **Screenshots are now visible during QA and browse sessions.** When opengstack takes screenshots, they now show up as clickable image elements in your output — no more invisible `/tmp/browse-screenshot.png` paths you can't see. Works in `/qa`, `/qa-only`, `/plan-design-review`, `/qa-design-review`, `/browse`, and `/opengstack`.
...
- Added `bin/opengstack-slug` helper (5-line bash) with unit tests. Outputs `SLUG=` and `BRANCH=` lines, sanitizes `/` to `-`.
...
- Added `~/.opengstack-dev/plans/` as a local plans directory for long-range vision docs (not checked in). CLAUDE.md and TODOS.md updated.
...
- **Clicking a dropdown option no longer hangs forever.** If an agent sees `@e3 [option] "Admin"` in a snapshot and runs `click @e3`, opengstack now auto-selects that option instead of hanging on an impossible Playwright click. The right thing just happens.
- **When click is the wrong tool, opengstack tells you.** Clicking an `<option>` via CSS selector used to time out with a cryptic Playwright error. Now you get: `"Use 'browse select' instead of 'click' for dropdown options."`
...
- **`/opengstack-upgrade` always checks for real.** Running `/opengstack-upgrade` directly now bypasses the cache and does a fresh check against GitHub. No more "you're already on the latest" when you're not.

### For contributors

- Split `last-update-check` cache TTL: 60 min for `UP_TO_DATE`, 720 min for `UPGRADE_AVAILABLE`.
- Added `--force` flag to `bin/opengstack-update-check` (deletes cache file before checking).
...
- **Every question is now crystal clear, every time.** You used to need 3+ sessions running before opengstack would give you full context and plain English explanations. Now every question — even in a single session — tells you the project, branch, and what's happening, explained simply enough to understand mid-context-switch. No more "sorry, explain it to me more simply."
- **Branch name is always correct.** opengstack now detects your current branch at runtime instead of relying on the snapshot from when the conversation started. Switch branches mid-session? opengstack keeps up.
...
- opengstack-upgrade template: explicit cross-step prose for variable references between bash blocks.

## 0.4.1 — 2026-03-16

- **opengstack now notices when it screws up.** Turn on contributor mode (`opengstack-config set OpenGStack_contributor true`) and opengstack automatically writes up what went wrong — what you were doing, what broke, repro steps. Next time something annoys you, the bug report is already written. Fork opengstack and fix it yourself.
- **Juggling multiple sessions? opengstack keeps up.** When you have 3+ opengstack windows open, every question now tells you which project, which branch, and what you were working on. No more staring at a question thinking "wait, which window is this?"
- **Every question now comes with a recommendation.** Instead of dumping options on you and making you think, opengstack tells you what it would pick and why. Same clear format across every skill.
...
### Added
- **`bin/opengstack-config` CLI** — simple get/set/list interface for `~/.opengstack/config.yaml`. Used by update-check and upgrade skill for persistent settings (auto_upgrade, update_check).
...
- **Auto-upgrade mode** — set `auto_upgrade: true` in config or `OpenGStack_AUTO_UPGRADE=1` env var to skip the upgrade prompt and update automatically.
- **4-option upgrade prompt** — "Yes, upgrade now", "Always keep me up to date", "Not now" (snooze), "Never ask again" (disable).
- **Vendored copy sync** — `/opengstack-upgrade` now detects and updates local vendored copies in the current project after upgrading the primary install.
- 25 new tests: 11 for opengstack-config CLI, 14 for snooze/config paths in update-check.

### Changed
- README upgrade/troubleshooting sections simplified to reference `/opengstack-upgrade` instead of long paste commands.
...
- **Greptile escalation detection** — explicit algorithm to detect prior opengstack replies on comment threads and auto-escalate to Tier 2.
...
### Fixed
- **`.gitignore` append failures silently swallowed** — `ensureStateDir()` bare `catch {}` replaced with ENOENT-only silence; non-ENOENT errors (EACCES, ENOSPC) logged to `.OpenGStack/browse-server.log`.
...
### Added
- **E2E observability** — heartbeat file (`~/.opengstack-dev/e2e-live.json`), per-run log directory (`~/.opengstack-dev/e2e-runs/{runId}/`), progress.log, per-test NDJSON transcripts, persistent failure transcripts. All I/O non-fatal.
...
- **Eval persistence** — results saved to `~/.opengstack-dev/evals/` with auto-comparison against previous run.
...
- Stale servers from global install no longer shadow local changes — removed legacy `~/.claude/skills/opengstack` fallback from `resolveServerScript()`
- Crash log path references updated from `/tmp/` to `.OpenGStack/`
...
- **Project-local browse state** — state file, logs, and all server state now live in `.OpenGStack/` inside the project root (detected via `git rev-parse --show-toplevel`). No more `/tmp` state files.
...
### Changed
- State file location: `.OpenGStack/browse.json` (was `/tmp/browse-server.json`)
- Log files location: `.OpenGStack/browse-{console,network,dialog}.log` (was `/tmp/browse-*.log`)
...
- Legacy fallback to `~/.claude/skills/opengstack/browse/src/server.ts`
- `DEVELOPING_opengstack.md` (renamed to CONTRIBUTING.md)
