...
description: |
 ...
 Proactively suggest when the user is about to merge or land code changes. (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Edit
...
```bash
~/.claude/skills/opengstack/bin/opengstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/opengstack/bin/opengstack-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
