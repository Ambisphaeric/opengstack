...
export class BrowserManager {
 ...
 /**
 * Find the opengstack Chrome extension directory.
 ...
 */
 ...
 // Global opengstack install
 path.join(process.env.HOME || '', '.claude', 'skills', 'OpenGStack', 'extension'),
 // Git repo root (detected via BROWSE_STATE_FILE location)
 (() => {
 const stateFile = process.env.BROWSE_STATE_FILE || '';
 if (stateFile) {
 const repoRoot = path.resolve(path.dirname(stateFile), '..');
 return path.join(repoRoot, '.claude', 'skills', 'OpenGStack', 'extension');
 }
 return '';
 })(),
 ].filter(Boolean);
 ...
 */
 ...
 console.error('[browse] Console/network logs flushed to .OpenGStack/browse-*.log');
 process.exit(1);
 });
 ...
 // ─── Headed Mode ─────────────────────────────────────────────
 /**
 * Launch Playwright's bundled Chromium in headed mode with the opengstack
 ...
 */
 ...
 // Find the opengstack extension directory for auto-loading
 ...
 const fs = require('fs');
 const path = require('path');
 const userDataDir = path.join(process.env.HOME || '/tmp', '.OpenGStack', 'chromium-profile');
 ...
 if (document.getElementById('opengstack-ctrl')) return;

 const topLine = document.createElement('div');
 topLine.id = 'opengstack-ctrl';
 topLine.style.cssText = `
 position: fixed; top: 0; left: 0; right: 0; height: 2px;
 background: linear-gradient(90deg, #F59E0B, #FBBF24, #F59E0B);
 background-size: 200% 100%;
 animation: opengstack-shimmer 3s linear infinite;
 ...
 style.textContent = `
 @keyframes opengstack-shimmer {
 0% { background-position: 200% 0; }
 100% { background-position: -200% 0; }
 }
 @media (prefers-reduced-motion: reduce) {
 #opengstack-ctrl { animation: none !important; }
 }
 `;
