/**
 * opengstack CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 * 1. Read .OpenGStack/browse.json for port + token
...
function cleanupLegacyState(): void {
 // No legacy state on Windows — /tmp and `ps` don't exist, and opengstack
...
async function main() {
 ...
 console.log(`opengstack browse — Fast headless browser for AI coding agents
...
Refs: After 'snapshot', use @e1, @e2... as selectors:
 ...
 const agentQueue = path.join(process.env.HOME || '/tmp', '.OpenGStack', 'sidebar-agent-queue.jsonl');
