/**
 * opengstack browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 * State file: <project-root>/.OpenGStack/browse.json (set via BROWSE_STATE_FILE env)
 * Log files: <project-root>/.OpenGStack/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
 ...
 const lines = ['opengstack browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.OpenGStack', 'sidebar-sessions');
...
function findBrowseBin(): string {
 ...
 path.resolve(__dirname, '..', '..', '.claude', 'skills', 'OpenGStack', 'browse', 'dist', 'browse'),
 path.join(process.env.HOME || '', '.claude', 'skills', 'OpenGStack', 'browse', 'dist', 'browse'),
 ];
...
function shortenPath(str: string): string {
 ...
 .replace(/\.claude\/skills\/opengstack\//g, '')
...
function createWorktree(sessionId: string): string | null {
 ...
 const worktreeDir = path.join(process.env.HOME || '/tmp', '.OpenGStack', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null, forTabId?: number | null): void {
 ...
 const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.OpenGStack', 'sidebar-agent-queue.jsonl');
 const OpenGStackDir = path.dirname(agentQueue);
 ...
 try {
 fs.mkdirSync(OpenGStackDir, { recursive: true });
...
async function shutdown() {
 ...
 const profileDir = path.join(process.env.HOME || '/tmp', '.OpenGStack', 'chromium-profile');
