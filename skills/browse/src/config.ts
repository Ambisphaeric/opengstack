 ...
 * 2. git rev-parse --show-toplevel → projectDir/.OpenGStack/
...
export function resolveConfig(
 ...
 projectDir = path.dirname(stateDir); // parent of .OpenGStack/
 } else {
 projectDir = getGitRoot() || process.cwd();
 stateDir = path.join(projectDir, '.OpenGStack');
...
/**
 * Create the .OpenGStack/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
 ...
 // Ensure .OpenGStack/ is in the project's .gitignore
 const gitignorePath = path.join(config.projectDir, '.gitignore');
 try {
 const content = fs.readFileSync(gitignorePath, 'utf-8');
 if (!content.match(/^\.OpenGStack\/?$/m)) {
 const separator = content.endsWith('\n') ? '' : '\n';
 fs.appendFileSync(gitignorePath, `${separator}.OpenGStack/\n`);
 }
 } catch (err: any) {
 if (err.code !== 'ENOENT') {
