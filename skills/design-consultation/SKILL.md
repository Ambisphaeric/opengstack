...
description: |
 ...
 design system or DESIGN.md. (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Write
...
ls ~/.opengstack/projects/$SLUG/*office-hours* 2>/dev/null | head -5
...
**Find the opengstack designer (optional — enables AI mockup generation):**
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the opengstack designer is available.
...
```bash
eval "$(~/.claude/skills/opengstack/bin/opengstack-slug 2>/dev/null)"
_DESIGN_DIR=~/.opengstack/projects/$SLUG/designs/design-system-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
