...
version: 1.0.0
description: |
 Manage project learnings. Review, search, prune, and export what opengstack
...
see what opengstack has learned across sessions on this project, search for relevant
...
~/.claude/skills/opengstack/bin/opengstack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
opengstack will automatically capture patterns, pitfalls, and insights it discovers."
...
```bash
eval "$(~/.claude/skills/opengstack/bin/opengstack-slug 2>/dev/null)"
~/.claude/skills/opengstack/bin/opengstack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(~/.claude/skills/opengstack/bin/opengstack-slug 2>/dev/null)"
~/.claude/skills/opengstack/bin/opengstack-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(~/.claude/skills/opengstack/bin/opengstack-slug 2>/dev/null)"
~/.claude/skills/opengstack/bin/opengstack-learnings-search --limit 50 2>/dev/null
```
...
```bash
eval "$(~/.claude/skills/opengstack/bin/opengstack-slug 2>/dev/null)"
OPENGSTACK_HOME="${OPENGSTACK_HOME:-$HOME/.OpenGStack}"
LEARN_FILE="$OPENGSTACK_HOME/projects/$SLUG/learnings.jsonl"
...
```bash
~/.claude/skills/opengstack/bin/opengstack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
