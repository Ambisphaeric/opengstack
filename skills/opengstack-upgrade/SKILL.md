---
name: opengstack-upgrade
version: 1.1.0
description: |
 Upgrade opengstack to the latest version. Detects global vs vendored install,
 runs the upgrade, and shows what's new. Use when asked to "upgrade OpenGStack",
 "update OpenGStack", or "get latest version".
allowed-tools:
 - Bash
 - Read
 - Write
...
[ -z "$_AUTO" ] && _AUTO=$(~/.claude/skills/opengstack/bin/opengstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.claude/skills/opengstack/bin/opengstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.claude/skills/opengstack/bin/opengstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.claude/skills/opengstack/bin/opengstack-config set update_check true` to re-enable."
...
TMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com.git "$TMP_DIR/opengstack"
...
```bash
~/.claude/skills/opengstack/bin/opengstack-update-check --force 2>/dev/null || \
.claude/skills/opengstack/bin/opengstack-update-check --force 2>/dev/null || true
```
