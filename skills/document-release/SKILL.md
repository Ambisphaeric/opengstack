...
description: |
 ...
 Proactively suggest after a PR is merged or code is shipped. (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Write
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.OpenGStack/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not opengstack-specific):
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/opengstack-pr-body-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/opengstack-pr-body-$$.md
```
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/opengstack-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/opengstack-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
...
```bash
rm -f /tmp/opengstack-pr-body-$$.md
```
