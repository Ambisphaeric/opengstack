...
description: |
 ...
 Use before /plan-ceo-review or /plan-eng-review. (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Grep
...
4. **List existing design docs for this project:**
 ...
 ls -t ~/.opengstack/projects/$SLUG/*-design-*.md 2>/dev/null
 ```
...
```bash
setopt +o nomatch 2>/dev/null || true # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.opengstack/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.opengstack/projects/`.
...
```bash
setopt +o nomatch 2>/dev/null || true # zsh compat
PRIOR=$(ls -t ~/.opengstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.opengstack/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
The design doc at `~/.opengstack/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
