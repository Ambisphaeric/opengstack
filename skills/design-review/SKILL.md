...
description: |
 ...
 wants to polish the look of a live site. (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Write
...
**Find the opengstack designer (optional — enables target mockup generation):**
...
REPORT_DIR=~/.opengstack/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
```
~/.opengstack/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
If the opengstack designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(~/.claude/skills/opengstack/bin/opengstack-slug 2>/dev/null)" && mkdir -p ~/.opengstack/projects/$SLUG
```
Write a one-line summary to `~/.opengstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
