...
description: |
 ...
 what it could look like. (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Glob
...
_PREV=$(find ~/.opengstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find ~/.opengstack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output ~/.opengstack/...`
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
 to `~/.opengstack/projects/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
