...
description: |
 ...
 "watch production", "verify deploy". (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Write
...
mkdir -p .OpenGStack/canary-reports
mkdir -p .OpenGStack/canary-reports/baselines
mkdir -p .OpenGStack/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".OpenGStack/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.OpenGStack/canary-reports/baseline.json`:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".OpenGStack/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".OpenGStack/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots: .OpenGStack/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.OpenGStack/canary-reports/{date}-canary.md` and `.OpenGStack/canary-reports/{date}-canary.json`.
