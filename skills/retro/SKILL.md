...
description: |
 ...
 Proactively suggest at the end of a work week or sprint. (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Write
...
**Greptile signal (if history exists):** Read `~/.opengstack/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
```
 EUREKA /office-hours (branch: "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
 EUREKA /plan-eng-review (branch: "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
```bash
~/.claude/skills/opengstack/bin/opengstack-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```json
{
 ...
 "authors": {
 "": { "commits": 32, "insertions": 2400, "deletions": 300, "test_ratio": 0.41, "top_area": "browse/" },
...
**Note:** Only include the `greptile` field if `~/.opengstack/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
cat ~/.opengstack/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the opengstack directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/opengstack-discover-stderr
```

Read the stderr output from `/tmp/opengstack-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
```bash
setopt +o nomatch 2>/dev/null || true # zsh compat
ls -t ~/.opengstack/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.opengstack/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.opengstack/retros/global-${today}-${next}.json`:
...
```json
{
 ...
 "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: opengstack (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.opengstack/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
