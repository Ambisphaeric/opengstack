...
description: |
 ...
 should be reviewed before implementation. (OpenGStack)
allowed-tools:
 - Read
 - Edit
 - Grep
...
### The opengstack designer — YOUR PRIMARY TOOL

You have the **opengstack designer**, an AI mockup generator that creates real visual mockups
...
opengstack designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the opengstack designer. This is how we
...
`~/.opengstack/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The opengstack
...
- `mkdir -p ~/.opengstack/projects/$SLUG/designs/...`
...
_DESIGN_DIR=~/.opengstack/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The opengstack designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
**Log the result:**
```bash
~/.claude/skills/opengstack/bin/opengstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/opengstack-ideal-<dimension>.png
```
...
|----------------|-------------|-----------|-------|
| [screen name] | ~/.opengstack/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
~/.claude/skills/opengstack/bin/opengstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
