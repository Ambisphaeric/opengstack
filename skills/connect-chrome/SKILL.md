...
version: 0.1.0
description: |
 Launch real Chrome controlled by opengstack with the Side Panel extension auto-loaded.
...
Connect Claude to a visible Chrome window with the opengstack extension auto-loaded.
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.OpenGStack/browse.json" ]; then
 _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.OpenGStack/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
 [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
 sleep 1
 [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
 rm -f "$(git rev-parse --show-toplevel)/.OpenGStack/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.OpenGStack/chromium-profile"
...
- The opengstack Chrome extension auto-loaded via `launchPersistentContext`
...
The `connect` command auto-discovers the extension from the opengstack install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.OpenGStack/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/opengstack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.claude/skills/opengstack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills/opengstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills/opengstack/extension"
...
> Chrome is launched with opengstack control. You should see Playwright's Chromium
...
> already show the opengstack icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **opengstack browse** → click the **pin icon**
> 3. Click the pinned **opengstack icon** in the toolbar
...
> 2. Look for **"opengstack browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the opengstack icon
...
> - Run any opengstack skill (`/qa`, `/design-review`, `/benchmark`) and watch
