...
description: |
 ...
 "land it", "ship it to production". (OpenGStack)
allowed-tools:
 - Bash
 - Read
 - Write
...
if [ ! -f ~/.opengstack/projects/$SLUG/land-deploy-confirmed ]; then
 echo "FIRST_RUN"
else
 # Check if deploy config has changed since confirmation
 SAVED_HASH=$(cat ~/.opengstack/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
```bash
~/.claude/skills/opengstack/bin/opengstack-review-read 2>/dev/null
```
...
```bash
mkdir -p ~/.opengstack/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.opengstack/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
ls -t ~/.opengstack-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true # zsh compat
ls -t ~/.opengstack-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `opengstack-diff-scope` to classify the changes:

```bash
eval $(~/.claude/skills/opengstack/bin/opengstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".OpenGStack/deploy-reports/post-deploy.png"
```
...
```bash
mkdir -p .OpenGStack/deploy-reports
```
...
Save report to `.OpenGStack/deploy-reports/{date}-pr{number}-deploy.md`.
