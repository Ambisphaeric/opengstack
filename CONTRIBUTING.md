# Contributing to opengstack

Thanks for wanting to make opengstack better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

opengstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/opengstack/` (your global install). But when you're developing opengstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.

That's what dev mode does. It symlinks your repo into the local `.claude/skills/` directory so Claude Code reads skills straight from your checkout.

```bash
git clone <repo> && cd opengstack
...
Contributor mode turns opengstack into a self-improving tool. Enable it and Claude Code
will periodically reflect on its opengstack experience — rating it 0-10 at the end of
each major workflow step. When something isn't a 10, it thinks about why and files
a report to `~/.opengstack/contributor-logs/` with what happened, repro steps, and what
would make it better.

```bash
~/.claude/skills/opengstack/bin/opengstack-config set OpenGStack_contributor true
```

The logs are for **you**. When something bugs you enough to fix, the report is
already written. Fork OpenGStack, symlink your fork into the project where you hit
the issue, fix it, and open a PR.

### The contributor workflow

1. **Use opengstack normally** — contributor mode reflects and logs issues automatically
2. **Check your logs:** `ls ~/.opengstack/contributor-logs/`
3. **Fork and clone OpenGStack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
 ```bash
 # In your core project (the one where opengstack annoyed you)
 ln -sfn /path/to/your/opengstack-fork .claude/skills/OpenGStack
 cd .claude/skills/opengstack && bun install && bun run build && ./setup
 ```
 Setup creates the per-skill symlinks (`qa -> OpenGStack/qa`, etc.) and asks your
 prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using OpenGStack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix opengstack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ opengstack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on opengstack inside the opengstack repo

When you're editing opengstack skills and want to test them by actually using opengstack
...
```
OpenGStack/ <- your working tree
├── .claude/skills/ <- created by dev-setup (gitignored)
│ ├── opengstack -> ../../ <- symlink back to repo root
│ ├── review -> OpenGStack/review <- short names (default)
│ ├── ship -> OpenGStack/ship <- or opengstack-review, opengstack-ship if --prefix
...
Skill symlink names depend on your prefix setting (`~/.opengstack/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/opengstack-review`, `/opengstack-ship`).
...
When E2E tests run, they produce machine-readable artifacts in `~/.opengstack-dev/`:
...
Artifacts are never cleaned up — they accumulate in `~/.opengstack-dev/` for post-mortem debugging and trend analysis.
...
opengstack generates SKILL.md files for two hosts: **Claude** (`.claude/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Aspect | Claude | Codex |
|--------|--------|-------|
| Output directory | `{skill}/SKILL.md` | `.agents/skills/opengstack-{skill}/SKILL.md` (generated at setup, gitignored) |
| Frontmatter | Full (name, description, allowed-tools, hooks, version) | Minimal (name + description only) |
| Paths | `~/.claude/skills/opengstack` | `$OpenGStack_ROOT` (`.agents/skills/opengstack` in a repo, otherwise `~/.codex/skills/opengstack`) |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/opengstack`. `bin/dev-teardown` restores the global one.
...
**This is the recommended way to develop opengstack.** Symlink your opengstack checkout
...
```bash
# In your core project (not the opengstack repo)
ln -sfn /path/to/your/opengstack-checkout .claude/skills/OpenGStack
```

### Step 2: Run setup to create per-skill symlinks

The `OpenGStack` symlink alone isn't enough. Claude Code discovers skills through
individual symlinks (`qa -> OpenGStack/qa`, `ship -> OpenGStack/ship`, etc.), not through
the `OpenGStack/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/opengstack && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/opengstack-qa`).
Your choice is saved to `~/.opengstack/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/opengstack/`:

```bash
rm .claude/skills/OpenGStack
```

The per-skill symlinks (`qa`, `ship`, etc.) still point to `OpenGStack/...`, so they'll
resolve to the global install automatically.

### Switching prefix mode

If you vendored opengstack with one prefix setting and want to switch:

```bash
cd .claude/skills/opengstack && ./setup --no-prefix # switch to /qa, /ship
cd .claude/skills/opengstack && ./setup --prefix # switch to /opengstack-qa, /opengstack-ship
```
