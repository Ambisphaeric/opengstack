...
This document covers the command reference and internals of OpenGStack's headless browser.
...
OpenGStack's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.OpenGStack/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.OpenGStack/browse-console.log`
- Network: `.OpenGStack/browse-network.log`
- Dialog: `.OpenGStack/browse-dialog.log`
...
The window has a subtle green shimmer line at the top edge and a floating "opengstack" pill in the bottom-right corner so you always know which Chrome window is being controlled.
...
# Click the opengstack icon in toolbar → Open Side Panel
```
...
```bash
bin/opengstack-extension # opens chrome://extensions, copies path to clipboard
```
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
 - Global install: `~/.claude/skills/opengstack/extension`
 - Dev/source: `<opengstack-repo>/extension`
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "opengstack browse"
6. **Set the port:** Click the opengstack icon → enter the port from `$B status` or `.OpenGStack/browse.json`
7. **Open Side Panel:** Click the opengstack icon → "Open Side Panel"
...
| **Connection pill** | Small "opengstack" pill in the bottom-right corner of every page when connected |
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.OpenGStack/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.OpenGStack/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.OpenGStack/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.OpenGStack/browse.json` | Path to state file (CLI passes to server) |
...
| **opengstack browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. opengstack burns zero.
...
opengstack skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.OpenGStack/browse.json`, sends HTTP to the server, prints response. |
...
1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/opengstack && git pull`
3. Rebuild: `cd ~/.claude/skills/opengstack && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/opengstack/browse/dist/browse`
